...
function tokenizeTextNodes(nodes) {
    return nodes.flatMap((node, nodeIndex) => (node.data.match(TOKEN_PATTERN) ?? []).map((value) => ({ value, nodeIndex })));
}
...
function proportionalNodeMap(original, revised) {
...
function alignParagraphs(original, revised) {
