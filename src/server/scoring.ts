...
function countRegexMatches(text: string, regex: RegExp): string[] {
  const matches = text.match(regex) ?? [];
  return matches.map((match) => normalizeWhitespace(match)).filter(Boolean);
}

function sampleEvidence(values: string[], max = 4): string[] {
...
function patternSignals(normalized: string, wordCount: number): SignalDraft[] {
...
export function detectAiSignals(text: string): { probability: number; signals: AiSignal[] } {
  ...
  const effectiveSafeguardScore =
    academicAiCluster || (wordCount < 180 && evidenceSignals.length >= 4)
  ...
  const strongAverage =
    strongSuspicionSignals.length === 0
      ? 0
